...
export default function CartTotal({ breakdown, isLoading, refetch }: Props) {
      ...
      paystackInstance.resumeTransaction(access_code, {
        onSuccess: async (data) => {
          try {
            ...
            navigate({ to: "/profile/orders", search: ((prev: any) => ({ ...prev })) as any });
          } catch (err: any) {
  ...
  const isFreeShipping = breakdown?.isFreeShipping;
    ...
    <div className="card bg-base-100 border border-base-200 shadow-xs sticky top-4">
      <div className="card-body gap-4 p-5">
        ...
        <div className="flex flex-col gap-2.5 text-sm">
          ...
          {distanceKm && distanceKm > 0 && !isFreeShipping ? (
            <p className="text-[11px] text-base-content/40 -mt-1">
              Estimated: {distanceKm} km (₦185/km, min ₦1,000)
            </p>
          ) : !isFreeShipping ? (
            <p className="text-[11px] text-base-content/40 -mt-1">
              Calculated at ₦185/km (min ₦1,000)
            </p>
          ) : null}
