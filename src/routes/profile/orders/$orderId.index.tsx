...
import OrderDeliveryCard from "#/routes/profile/-components/OrderDeliveryCard";
import type {
  CategoryResponse,
  LogisticsResponse,
  OrderItemsResponse,
  ...
  loader: async ({ params }) => {
    return await ssr_pb()
      ...
      .getOne<OrderWithExpand>(params.orderId, {
        expand:
          "orderItems,orderItems.originalProduct,orderItems.originalProduct.category.parent,logisitics",
      });
  },
});
...
function RouteComponent() {
  ...
  return (
    <div className="space-y-6">
      ...
      <button
        type="button"
        onClick={() => nav({ to: "/profile/orders" })}
        className="btn btn-ghost btn-sm gap-2 text-xs text-base-content/60 hover:text-base-content rounded-xl"
      >
      ...
      <PageLoader query={query}>
        {(order) => {
          const items = order.expand?.orderItems || [];
          ...
          return (
            <div className="space-y-6">
              {/* Main Order Header Hero Banner */}
              <div
                className={`card bg-gradient-to-br ${
                  statusGradient[order.status || "pending"] ||
                  statusGradient.pending
                } border border-base-200/80 shadow-xs overflow-hidden`}
              >
                <div className="card-body p-6 md:p-8 space-y-6">
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div className="space-y-1">
                      <div className="flex items-center gap-2.5">
                        <span
                          className={`badge ${
                            ...
                            "badge-ghost"
                          } font-bold text-xs uppercase tracking-wider py-2.5 px-3 rounded-lg`}
                        >
                      ...
                      <h1 className="text-2xl md:text-3xl font-black tracking-tight text-base-content font-mono">
              ...
              {/* Delivery & Security Verification Card */}
              <OrderDeliveryCard order={order as any} />
                  ...
                  <div className="space-y-2 text-xs text-base-content/70">
                    ...
                    <div className="flex items-start gap-2 pt-2 border-t border-base-200">
                      ...
                      <div>
                        ...
                        <p className="text-base-content/50 mt-0.5">
                          Processed securely via Paystack with automated confirmation.
                        </p>
                      </div>
                    </div>
                  </div>
