...
export default function AllOrderItems({ items }: { items: ItemWithProduct[] }) {
    ...
    <div className="rounded-2xl overflow-hidden border border-base-200 shadow-sm">
        ...
        {(firstProduct?.mainColor || firstProduct?.secondaryColor) && (
          <div className="absolute top-3 right-3 flex gap-1.5">
            {firstProduct.mainColor && (
              <span className="size-6 rounded-full border-2 border-white/80 shadow" style={{ backgroundColor: firstProduct.mainColor }} />
            )}
            {firstProduct.secondaryColor && (
              <span className="size-6 rounded-full border-2 border-white/80 shadow" style={{ backgroundColor: firstProduct.secondaryColor }} />
            )}
          </div>
        )}
      ...
      <div className="bg-base-100 px-5 pb-5 -mt-2 flex flex-col gap-4">
        ...
        <div className="flex flex-col divide-y divide-base-200">
          {items.map((item, i) => {
            const product = (item.expand as any)?.originalProduct as ProductsResponse | undefined;
            ...
            return (
              <div key={item.id ?? i} className="flex items-center gap-3 py-3 first:pt-0 last:pb-0">
                {imgUrl ? (
                  <img src={imgUrl} alt={product?.title} className="size-12 rounded-xl object-cover shrink-0 bg-base-200" />
                ) : (
                  <div className="size-12 rounded-xl bg-base-200 shrink-0 flex items-center justify-center text-base-content/20">
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-semibold truncate">
                    {product?.title ?? <span className="italic text-base-content/30">Unknown</span>}
                  </p>
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-xs text-base-content/40">
                      ×{item.amount ?? 1} · ₦{(item.price ?? 0).toLocaleString()} each
                    </span>
                  </div>
                </div>
